...
const Amojonamientos = () => {
  ...
  const contarTrabajosPorEstadoContable = (estadoContableId) => {
    if (estadoContableId === 1) {
      // Para "Por cobrar", contar solo si el trabajo está en "Finalizado"
      return trabajos.filter(
        (trabajo) => trabajo.estado_contable === estadoContableId && trabajo.estado === 3
      ).length;
    }
    // Para otros estados contables, contar normalmente
  ...
  const renderTrabajosPorEstadoContable = (estadoContableId) => {
    const trabajosFiltrados = trabajos
      .filter(
        (trabajo) =>
          trabajo.estado_contable === estadoContableId &&
          (estadoContableId !== 1 || trabajo.estado === 3) // Solo mostrar "Por cobrar" si está en "Finalizado"
      )
      .sort((a, b) => b.num_trabajo - a.num_trabajo);
      ...
      >
        <div className="d-flex justify-content-between align-items-center mb-3">
          ...
          <h1
            ...
            style={{ fontSize: "24px" }}
          >
            Amojonamientos y DJCU - Progreso General
          </h1>
